...
public class LoginController extends BaseController {
	...
	private boolean isInBlackList(String ip) {
		Long orgNum = systemService
				.getCountForJdbc("select count(*) from t_s_black_list where ip =  '"
						+ ip + "'");
	...
	private void saveLoginSuccessInfo(HttpServletRequest req, TSUser user,
		...
		Client clientOld = ClientManager.getInstance().getClient(
				session.getId());
	...
	public String login(ModelMap modelMap, HttpServletRequest request,
		...
		if (user != null) {
			...
			modelMap.put("roleName", roles.length() > 3 ? roles.substring(0, 3)
					+ "..." : roles);
			modelMap.put("userName", user.getUserName().length() > 5 ? user
			...
			if ("ace".equals(sysTheme.getStyle())
					...
					|| "hplus".equals(sysTheme.getStyle())) {
			...
			/*
			 * 单点登录 - 登录需要跳转登录前页面，自己处理 ReturnURL 使用 HttpUtil.decodeURL(xx)
			 * 解码后重定向
			 */
	...
	public ModelAndView logout(HttpServletRequest request) {
		...
		try {
			systemService.addLog("用户" + user != null ? user.getUserName() : ""
					+ "已退出", Globals.Log_Type_EXIT, Globals.Log_Leavel_INFO);
		} catch (Exception e) {
	...
	private Map<Integer, List<TSFunction>> getFunctionMap(TSUser user) {
		...
		if (client.getFunctionMap() == null
				|| client.getFunctionMap().size() == 0) {
			Map<Integer, List<TSFunction>> functionMap = new HashMap<Integer, List<TSFunction>>();
			...
			if (loginActionlist.size() > 0) {
				...
				for (TSFunction function : allFunctions) {
					...
					if (!functionMap
							.containsKey(function.getFunctionLevel() + 0)) {
						functionMap.put(function.getFunctionLevel() + 0,
								new ArrayList<TSFunction>());
					}
					functionMap.get(function.getFunctionLevel() + 0).add(
							function);
				}
				// 菜单栏排序
	...
	private Map<String, TSFunction> getUserFunction(TSUser user) {
		...
		if (client.getFunctions() == null || client.getFunctions().size() == 0) {
			 ...
			 * log.info("role functions:  "+list.size()); for(TSFunction
			 * function:list){ loginActionlist.put(function.getId(),function); }
			 * List<TSFunction> list2 = systemService.findHql(hql2);
			 * log.info("org functions: "+list2.size()); for(TSFunction
			 * function:list2){ loginActionlist.put(function.getId(),function);
			 * }
			 */
			...
			StringBuilder hqlsb2 = new StringBuilder(
					"select distinct c from TSFunction c,TSRoleFunction rf,TSRoleOrg b,TSUserOrg a ")
			...
			List<TSFunction> list1 = systemService.findHql(hqlsb1.toString(),
					user.getId());
			List<TSFunction> list2 = systemService.findHql(hqlsb2.toString(),
					user.getId());
	...
	public String getPrimaryMenu() {
		List<TSFunction> primaryMenu = getFunctionMap(
				ResourceUtil.getSessionUser()).get(0);
		String floor = "";
		...
		for (TSFunction function : primaryMenu) {
			if (function.getFunctionLevel() == 0) {
				...
				if ("业务申请".equals(lang_context)) {
					...
					String ss = "<div style='width:67px;position: absolute;top:39px;text-align:center;color:#909090;font-size:13px;'><span style='letter-spacing:-1px;'>"
							+ lang_context + "</span></div>";
					floor += " <li style='position: relative;'>"
							+ ss
							+ "<img class='imag1' src='plug-in/login/images/ywsq.png' /> "
					...
					floor += " <li style='position: relative;'>"
							+ ss
							+ "<img class='imag1' src='plug-in/login/images/grbg.png' /> "
					...
					floor += " <li style='position: relative;'>"
							+ ss
							+ "<img class='imag1' src='plug-in/login/images/lcsj.png' /> "
					...
					floor += " <li style='position: relative;'>"
							+ ss
							+ "<img class='imag1' src='plug-in/login/images/zdybd.png' /> "
					...
					floor += " <li style='position: relative;'>"
							+ ss
							+ "<img class='imag1' src='plug-in/login/images/msg.png' /> "
					...
					if (lang_context.length() >= 5 && lang_context.length() < 7) {
						s = "<div style='width:67px;position: absolute;top:39px;text-align:center;color:#909090;font-size:13px;'><span style='letter-spacing:-1px;'>"
								+ lang_context + "</span></div>";
					} else if (lang_context.length() < 5) {
						s = "<div style='width:67px;position: absolute;top:39px;text-align:center;color:#909090;font-size:13px;'>"
								+ lang_context + "</div>";
	...
	public String getPrimaryMenuDiy() {
		// 取二级菜单
		List<TSFunction> primaryMenu = getFunctionMap(
				ResourceUtil.getSessionUser()).get(1);
		String floor = "";
		if (primaryMenu == null) {
			return floor;
		}
			...
			if (menuString.contains(function.getFunctionName())) {
				if (function.getFunctionLevel() == 1) {
					...
					if ("申请".equals(lang_key)) {
						...
						// 其他的为默认通用的图片模式
						String s = "";
						if (lang_context.length() >= 5
								&& lang_context.length() < 7) {
